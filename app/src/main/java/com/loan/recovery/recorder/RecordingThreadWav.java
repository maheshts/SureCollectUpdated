...
import static com.loan.recovery.util.AppConstants.ERROR;
...
class RecordingThreadWav extends RecordingThread implements Runnable {
    ...
    public void run() {
        try (FileOutputStream outputStream = new
            ...
            Log.e(ERROR, "Error while writing temp pcm file: " + e.getMessage());
            if (!tmpFile.delete())
                Log.e(ERROR, "Cannot delete incomplete temp pcm file.");
    ...
    static class CopyPcmToWav implements Runnable {
        ...
        public void run() {
            ...
            try (FileInputStream tmpInput = new FileInputStream(tmpFile);
                ...
                Log.e(ERROR, "Error while copying temp pcm to wav file: " + e.getMessage());
                if (!tmpFile.delete())
                    Log.e(ERROR, "Error while deleting temp pcm file on exception.");
                if (!wavFile.delete())
                    Log.e(ERROR, "Error while deleting wav file on exception.");
                recorder.setHasError();
                notifyOnError(context);
            } finally {
                if (!tmpFile.delete())
                    Log.e(ERROR, "Error while deleting temp pcm file on normal exit.");
            }
        }

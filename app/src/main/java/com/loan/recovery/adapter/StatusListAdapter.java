...
public class StatusListAdapter extends ArrayAdapter<Status> {
    ...
    public StatusListAdapter(@NonNull Context context, int resource, List<Status> data) {
    ...
    public View getView(final int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        rowView.setOnClickListener(v -> {
            if (mClickListener != null) mClickListener.onItemClick(status);
        });
        AppCompatTextView tvName = rowView.findViewById(R.id.tvName);
        tvName.setText(status.getStatusName());

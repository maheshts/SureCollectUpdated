...
public class CasesRecyclerAdapter extends RecyclerView.Adapter<CasesRecyclerAdapter.CaseViewHolder>
    ...
    public CasesRecyclerAdapter(List<CaseData> caseData, HomeActivity context, HomeFragment fragment) {
    ...
    public CaseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.case_item_layout, parent, false);
    ...
    public void onBindViewHolder(final CaseViewHolder holder, final int position) {
        try {
            CaseData caseData = caseDataList.get(position);
            String strPhone = caseData.getPhoneNumber();
            String aiqNumber = application.getPhoneNumbersList().get(0).getUserPhone();
            holder.tvName.setText(": " + caseData.getCustomerName());
            holder.textViewName.setText(caseData.getCustomerName());
            holder.tvDueAmount.setText(": \u20B9 " + caseData.getDueAmount());
            holder.tvPrinciple.setText(": \u20B9 " + caseData.getOdPrincipal());
            holder.tvPhoneNumber.setText(": " + strPhone);
            holder.tvAgreementId.setText(": " + caseData.getPartnerCaseId());
            holder.tvScore.setText(": " + caseData.getPropensityScore());
            holder.tvCity.setText(": " + ((caseData.getOffCityName() != null) ? caseData.getOffCityName() : "NA"));
            holder.tvStatus.setText(": " + caseData.getStatusName());
            ...
            holder.btnCallOtherContact1.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Toast.makeText(context, caseData.getAltPhoneNumber1(), Toast.LENGTH_SHORT).show();
                    try {
...
//                }
    ...
    private void buildAlertMessageNoGps() {
        ...
        final android.app.AlertDialog dialogConfirm = new android.app.AlertDialog.Builder(context).create();
